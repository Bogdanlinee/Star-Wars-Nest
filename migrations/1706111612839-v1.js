...
module.exports = class V11706111612839 {
    name = 'V11706111612839'

    async up(queryRunner) {
        await queryRunner.query(`CREATE TABLE \`image_person\`
                                 (
        ...
        await queryRunner.query(`CREATE TABLE \`planet\`
                                 (
                                     \`id\`              int          NOT NULL AUTO_INCREMENT,
        ...
        await queryRunner.query(`CREATE TABLE \`starship\`
                                 (
                                     \`id\`                     int          NOT NULL AUTO_INCREMENT,
                                     \`name\`                   varchar(255) NOT NULL,
        ...
        await queryRunner.query(`CREATE TABLE \`film\`
                                 (
                                     \`id\`            int          NOT NULL AUTO_INCREMENT,
        ...
        await queryRunner.query(`CREATE TABLE \`planet_film\`
                                 (
                                     \`planet_id\` int NOT NULL,
                                     \`film_id\`   int NOT NULL,
                                     INDEX         \`IDX_0eb506799ac4bf5f78570a2450\` (\`planet_id\`),
                                     INDEX         \`IDX_092b0534049588c9622b44874b\` (\`film_id\`),
                                     PRIMARY KEY (\`planet_id\`, \`film_id\`)
                                 ) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`planet_species\`
                                 (
                                     \`planet_id\`  int NOT NULL,
                                     \`species_id\` int NOT NULL,
                                     INDEX          \`IDX_b1dd5afead177d1c0d2d3a79b9\` (\`planet_id\`),
                                     INDEX          \`IDX_95094b6b3dfb31859956d52396\` (\`species_id\`),
                                     PRIMARY KEY (\`planet_id\`, \`species_id\`)
                                 ) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`species_film\`
                                 (
                                     \`species_id\` int NOT NULL,
                                     \`film_id\`    int NOT NULL,
                                     INDEX          \`IDX_5faf4f27306eebd5a407e6c26e\` (\`species_id\`),
                                     INDEX          \`IDX_78aad5950b7e32ce123b5f3fed\` (\`film_id\`),
                                     PRIMARY KEY (\`species_id\`, \`film_id\`)
                                 ) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`species_person\`
                                 (
                                     \`species_id\` int NOT NULL,
                                     \`person_id\`  int NOT NULL,
                                     INDEX          \`IDX_5f4a67c6e3c15e881a21a419ac\` (\`species_id\`),
                                     INDEX          \`IDX_d558e4914b65efd642b9f66f13\` (\`person_id\`),
                                     PRIMARY KEY (\`species_id\`, \`person_id\`)
                                 ) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`starships_person\`
                                 (
                                     \`starship_id\` int NOT NULL,
                                     \`person_id\`   int NOT NULL,
                                     INDEX           \`IDX_21fc491395d026eb07a518b71e\` (\`starship_id\`),
                                     INDEX           \`IDX_a5772881beef0be99f472d608d\` (\`person_id\`),
                                     PRIMARY KEY (\`starship_id\`, \`person_id\`)
                                 ) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`starships_film\`
                                 (
                                     \`starship_id\` int NOT NULL,
                                     \`film_id\`     int NOT NULL,
                                     INDEX           \`IDX_199ec4b25699908a6d507dcce1\` (\`starship_id\`),
                                     INDEX           \`IDX_cb5e3484f0fe0a779049c1fe3e\` (\`film_id\`),
                                     PRIMARY KEY (\`starship_id\`, \`film_id\`)
                                 ) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`person_film\`
                                 (
                                     \`person_id\` int NOT NULL,
                                     \`film_id\`   int NOT NULL,
                                     INDEX         \`IDX_52aefcfd3f58e9091e43396872\` (\`person_id\`),
                                     INDEX         \`IDX_22f984b92dee71af3d7daa5f82\` (\`film_id\`),
                                     PRIMARY KEY (\`person_id\`, \`film_id\`)
                                 ) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`image_person\`
            ADD CONSTRAINT \`FK_41b33f410c401c115103b703969\` FOREIGN KEY (\`personId\`) REFERENCES \`person\` (\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`person\`
            ADD CONSTRAINT \`FK_bc15517e0ebbab3fd38edf4029a\` FOREIGN KEY (\`homeworld\`) REFERENCES \`planet\` (\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`planet_film\`
            ADD CONSTRAINT \`planet_film_planet_id\` FOREIGN KEY (\`planet_id\`) REFERENCES \`planet\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`planet_film\`
            ADD CONSTRAINT \`planet_film_film_id\` FOREIGN KEY (\`film_id\`) REFERENCES \`film\` (\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`planet_species\`
            ADD CONSTRAINT \`planet_species_planet_id\` FOREIGN KEY (\`planet_id\`) REFERENCES \`planet\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`planet_species\`
            ADD CONSTRAINT \`planet_species_species_id\` FOREIGN KEY (\`species_id\`) REFERENCES \`species\` (\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`species_film\`
            ADD CONSTRAINT \`species_film_species_id\` FOREIGN KEY (\`species_id\`) REFERENCES \`species\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`species_film\`
            ADD CONSTRAINT \`species_film_film_id\` FOREIGN KEY (\`film_id\`) REFERENCES \`film\` (\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`species_person\`
            ADD CONSTRAINT \`species_person_species_id\` FOREIGN KEY (\`species_id\`) REFERENCES \`species\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`species_person\`
            ADD CONSTRAINT \`species_person_person_id\` FOREIGN KEY (\`person_id\`) REFERENCES \`person\` (\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`starships_person\`
            ADD CONSTRAINT \`starship_person_starship_id\` FOREIGN KEY (\`starship_id\`) REFERENCES \`starship\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`starships_person\`
            ADD CONSTRAINT \`starship_person_person_id\` FOREIGN KEY (\`person_id\`) REFERENCES \`person\` (\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`starships_film\`
            ADD CONSTRAINT \`starship_film_starship_id\` FOREIGN KEY (\`starship_id\`) REFERENCES \`starship\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`starships_film\`
            ADD CONSTRAINT \`starship_film_film_id\` FOREIGN KEY (\`film_id\`) REFERENCES \`film\` (\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`person_film\`
            ADD CONSTRAINT \`person_film_character_id\` FOREIGN KEY (\`person_id\`) REFERENCES \`person\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`person_film\`
            ADD CONSTRAINT \`person_film_film_id\` FOREIGN KEY (\`film_id\`) REFERENCES \`film\` (\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
